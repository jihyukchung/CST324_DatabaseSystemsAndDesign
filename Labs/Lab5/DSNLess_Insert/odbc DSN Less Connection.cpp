...
void PrintCategories()
{
	...
	SQLHANDLE hstmt;
	SDWORD cbQual;
	const char* query = "SET IDENTITY_INSERT Categories ON; insert into Categories(CategoryID, CategoryName) values(6, 'ElectricGuitar'); SET IDENTITY_INSERT Categories OFF;";
	char id[100];
	char name[100];
	SQLRETURN retcode;
	...
	// {ODBC Driver 13 for SQL Server *.mdb}
	// {ODBC Driver 13 for SQL Server (*.mdb)}
	// {ODBC Driver 13 for SQL Server *.dbf}
	...
	// {SQL Native Client}
	// {SQL Server 14.0.3045.24 - jihyukchung}
	// {SQL Server 14.0.3045.24}
	// {Lab5_ODBC}

	sprintf((char *)InConnStr, "Driver={ODBC Driver 13 for SQL Server};Server=kelleytech.net,5433;Database=jihyukchungMyGuitarShop;Uid=jihyukchung;Pwd=jihyukchung;");
	...
	if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) 
	{
		...
		if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) 
		{
			...
			if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) 
			{
				...
				retcode = SQLDriverConnect( hdbc, desktopHandle, (unsigned char*)InConnStr, strlen((char*)InConnStr), (unsigned char *)OutConnStr, 255, &OutConnStrLen, SQL_DRIVER_NOPROMPT);
				...
				else
				{
					...
					do
					{
						ret = SQLGetDiagRec(SQL_HANDLE_DBC, hdbc, ++i, (unsigned char*)state, &native, (unsigned char*)text, sizeof(text), &len );
